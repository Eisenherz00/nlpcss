"""agent_local.py — Local version of the survey-item generation agent.

This file is identical to agent.py except that it loads a smaller model
from HuggingFace Hub (Qwen2.5-1.5B-Instruct, ~3 GB) so you can develop
and test on your own laptop without access to the LRZ GPU cluster.

Usage:
    python agent_local.py                       # uses default model
    MODEL_NAME=meta-llama/Llama-3-8B-Instruct python agent_local.py  # override
"""
...
# === MODEL CONFIG ===
# Default: small model that runs on a laptop (auto-downloads ~3 GB on first run).
# Override with the MODEL_NAME environment variable if you want a different model.
...
def load_model():
    """Load the tokenizer and model from HuggingFace Hub (auto-downloads on first run)."""
...
def _run(assertion, tokenizer, model, suffix=""):
    """Send a single assertion to the model and return the raw text response."""
...
def generate_item(assertion, tokenizer, model):
    """Generate a survey item JSON for a given assertion, with one retry on parse failure."""
